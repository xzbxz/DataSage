...
覆盖 delivery、receipt/collections、receivable、target、inventory、profit
六个经营域。它提供受治理证据的事实、诊断和分级建议，不是
...
利润域复用现有三个 DataSage 工具与官方执行链，不新增自由 SQL 工具。35项指标分为客户月度报表和出库单生命周期报表，提供已记录销售收入、毛利、毛利率及费用组成；不将报表更新等同会计结账。

| 口径 | 期间与支持维度 | 限制 |
|---|---|---|
| 客户月度利润 | 完整月范围、月度趋势；客户、部门、组织、业务员、地区及源表标签 | 客户同月多行全部汇总；源表没有产品/订单字段，不接受这些维度 |
| 出库单生命周期利润 | 按原出库日期选择订单；客户、产品、订单、部门、组织、记录业务员名称及源表标签 | 历史值可能随费用/退货更新；记录业务员名称不是唯一人员ID，不冒充客户月度经营利润 |

收入和毛利使用已记录字段。费用NULL不自动补零；完整金额未知时返回缺失数量、覆盖率和已知部分金额，全部缺失时已知部分也为空。毛利率按毛利合计/销售收入合计计算，公开分子分母；非零负收入保留符号，零收入或组成缺失时未定义。用户已确认客户报表额外扣减项为取消订单库存DDP金额；已包含在报表毛利内，不重复扣减。默认包含内部客户，并以实际筛选为准。

核验和解释的权威范围在 `plugins/datasage-query/contracts/profit-semantics.yaml`；本说明不复制完整公式。插件注册时固定合同快照，新提交需在受影响网关重新注册后供其使用；已有会话提示可能仍保留旧内容。
