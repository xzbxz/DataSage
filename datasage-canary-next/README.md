...
收入和毛利使用已记录字段。费用NULL不自动补零；完整金额未知时返回缺失数量、覆盖率和已知部分金额，全部缺失时已知部分也为空。毛利率按毛利合计/销售收入合计计算，公开分子分母；非零负收入保留符号，零收入或组成缺失时未定义。库存成本字段的业务归属未确认，不自动解释为取消订单损失。默认包含内部客户，并以实际筛选为准。
