...
维护时间不代表库存ETL刷新，读取日期也不是刷新日期。现存基线状态比较使用下节独立接口；基线产品范围净出库和精确旧批次去化仍未实现；不冻结、不注册定时任务、不写状态或发消息。HT/非HT范围、仓库/销售归属、冻结频率冲突及历史期末来源留待独立核验。
...
时间显示保持库端原值；当前UTC偏移不证明历史写入时区。frozen_at字段可能随更新改写，当前记录结构通过校验也不证明当初全量冻结且从未修改。没有历史期末证据时拒绝回填；基线产品范围净出库暂未接入。冻结、状态写入、定时和发送均不在本次能力内。
