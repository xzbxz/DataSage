...
inventory域的registered_slow_pool系列与普通库存、库龄、周转分开。候选由上游库存查询生成，人工只维护白名单；本插件不重定数量或标签。按已确认范围读取：来源库存数量大于10且排除白名单。这是明确的查询范围，未声明为现行公司所有滞销库存的唯一判定规则，也不换成未销售天数或10卷。

提供数量（按库存单位隔离）、原记录卷数、来源条目数、产品及规格标识去重数、可打折优先类别卷数（不含归入处理类的双标签记录）、正促销价覆盖及交集、未知类别卷数、促销价NULL缺失卷数。类别与正价覆盖可重叠，不能相加冒充总池；无匹配记录与分类确为0分开解释；缺失保留缺口和已知部分。

库存单位使用source_unit，不回退促销计价单位。这一组仅支持当前读取状态；周基线比较和独立月报使用下方对应受控接口。价格仅在私有编译中判定是否有正值，不公开价格、备注或维护人资料；货值及换算依据不足，暂不开放货值。

维护时间不代表库存ETL刷新，读取日期也不是刷新日期。现存基线状态比较使用下节独立接口；基线产品范围净出库使用下方独立接口；精确旧批次去化仍未实现；不冻结、不注册定时任务、不写状态或发消息。基线净出库已核对旧追踪范围和物理仓库归属；周基线原则上不重复冻结，正常重跑复用本周基线；本插件不写基线。独立月报已有下方专用来源，不用当前库存回填历史。
...
新增 inventory.registered_slow_monthly_groups、registered_slow_monthly_summary、registered_slow_monthly_net_outbound。月份使用calendar_month（或等价完整日历月time_range）；缺省为当前业务月。当月截止为本次读取，历史期末为目标月物理库存快照。

月初从上月inventory_barcode_detail_bymonth_dw，按已确认标签/仓库/组织及HCM、HN、BKK、IDK与各自-HT部门范围，四键汇总后数量>10；当月期末取当前ODS源行>10范围。两端统一使用同次当前白名单的产品、规格、仓库部门、库存单位组合，不用促销单位，不创建月初冻结。New参与总体变化、不入月初池战果。历史期末缺失不抹掉独立可用期间战果。指定周基线查月流水仍是补充，不能替代独立月报。

周接口与月接口共享出退资格、单位、业务键、实际时间和自身销售计算。普通战果是net_rolls；高折战果是high_net_rolls，取成交价严格>DDP×0.75的出库卷数减全部合格退货卷数。没有DDP正数附加门槛；NULL、异常价、负数与原精度保留。价格只在专用计算列中使用，不开放原价或自由SQL。

scope_*卷数总量和sales_*销售卷数总量在截断前计算，只跨数量单位合并同为卷的字段，数量不跨单位相加。销售普通净卷0但高折非0仍保留。正文和本机报表直接消费这些结果，不重算高折、不按普通净值过滤销售；仍是进展观察，不新增奖金规则。不同独立查询各有读取时点；同一销售查询自带同次总体，不能把独立库存变化当销售恒等式。

目录grouping同时说明可分组、必需和默认维度，区别于可筛选维度。普通库存三原始标签尚未全面扩展；登记类别不是原始可打折标签，正促销价不是低价成交或高折战果。
