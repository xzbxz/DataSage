...
inventory域增加 registered_slow_pool_baseline_net_outbound；baseline_week选择现存基线，默认当前业务周及之前最新已记录周。dimensions默认[unit]，可附加product、pool_sku、warehouse_department；同名metric_filters限定冻结范围。本接口不支持销售分组、任意时间窗、历史期末或状态筛选。

仅从现存基线去重后的产品、规格、仓库部门、可靠库存单位集合匹配流转，时间为实际记录冻结时间（含）至固定库端读取时间（不含），不补入当前New。单据范围沿用用户确认的旧追踪器谓词：来源部门LIKE '%-HT'或自身类型为bulk；HT不另设类型白名单。出库按自身bill_type，退货按自身sale_bill_type。出库须唯一关联已完成销售明细；退货须status=4、complnt_type=1、channel_type=1，两侧均排除内部客户。

退货取实际退入仓及自身事实部门，不追溯原出库仓或销售。当前仓库主数据仅校验部门一致性，不拿当前属性覆盖历史事实。两侧分别聚合再相减；同键多条基线不放大流量。未匹配、已排除、未知候选及缺时间记录计数保留，计数覆盖筛选后的基线产品/规格候选全范围，不能在各结果行重复相加。

结果包含毛出库、退货、数量和卷数的已知部分，以及recorded_net_quantity/recorded_net_rolls。单侧无记录时完整净数留空，已记录净数只表示已匹配且有数值记录的代数差，不证明另一侧业务为零；双侧有记录且覆盖条件通过才给完整记录净数。缺身份、单位、必要时间、有效性或仓库归属时保留部分未知；不跨单位相加，不用促销单位补缺，负净数不截零。没有合格基线或基线键不完整时失败。

该指标不能证明原冻结旧批次被销售：同产品后续新货也可能在范围内。记录冻结时间可更新，查询不能证明原始冻结从未删改；读数亦不证明上游数据已全部到齐。无写库、冻结、状态、定时或消息行为。
