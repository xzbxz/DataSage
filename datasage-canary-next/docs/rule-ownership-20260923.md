...
- 规则图必须无环：若 reference A 链接到 B，而 B 又回到 A，读取顺序就会改变口径归属；
  该测试断言整个规则引用图是 DAG（顺序无关）。
- references 不得自带 SQL 或公式定义：以「SELECT … FROM」模式与代码围栏为信号，
  防止方法材料变成第二套执行真源。
...
`test_host_compaction_e2e`（压缩后保留最新纠正，已在锁定宿主路径下通过）、
`test_business_contracts`（同一口径跨域一致）。

## 4. 结论边界

- 本表是维护索引，不是模型输入；它不改变任何合同、权限或工具面。
- 顺序无关、无环、无 SQL 是素材级证明；「模型实际按顺序读取会不会改口径」仍需真实回放，
  见 ARCHITECTURE 的门槛。
