...
| 文件读取与交付 | 既有本地受控报告与writer | 原特殊容器方案已退役；企微不提供任意文件/代码工具，受控导出回归保留，真实投递另验 |
...
TERMINAL环境、不自动启动Podman、不依赖refactor-work。`config.yaml`保持原有Git跟踪，仅提交非秘密选项与环境占位符；本机凭据、
生成入口及其备份继续Git忽略。不要把秘密或机器启动参数写入版本化源码。
