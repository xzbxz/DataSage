...
2. 合成回放时，将源记录映射为受控测试数据库或查询证据，用实际Profile走完整会话。不得把合成身份用于真实数据库；这些逻辑业务夹具尚不是六域物理SQL数据库种子。记录实际工具请求、返回证据、最终答案和多轮上下文。
...
| B07 | receivable, customer_risk | 已确认逾期明细 | {"overdue_total_rmb":60000,"amount_priority":"甲","age_priority":"乙"} |
...
| B12 | customer_risk | Top5缺少全量分母 | {"top5_subtotal_rmb":100000,"company_share":null} |
