...
class BaselineNetTests(unittest.TestCase):
    ...
    def test_empty_recorded_set_is_zero_without_artificial_missingness(self):
        ...
        self.assertEqual(10,f['metric_value']);self.assertEqual(0,f['return_quantity']);self.assertEqual(0,f['missing_value_count'])
        ...
        f=self.value();self.assertEqual(-4,f['metric_value']);self.assertEqual(0,f['gross_quantity']);self.assertEqual(0,f['missing_value_count'])
    ...
    def test_unknown_unit_identity_time_and_master_mismatch_are_coverage_gaps(self):
        ...
        f=self.value();self.assertIsNone(f['metric_value']);self.assertEqual(7,f['known_subset_value']);self.assertEqual(3,f['outbound_unknown_rows']);self.assertEqual(1,f['returns_unknown_rows']);self.assertEqual(1,f['outbound_missing_time_rows'])
    def test_legacy_ht_is_unrestricted_non_ht_bulk_only(self):
        ...
        f=self.value();self.assertEqual(1,f['outbound_excluded_rows']);self.assertEqual(1,f['outbound_unknown_rows']);self.assertEqual(4,f['known_subset_value'])
        ...
        f=self.value();self.assertEqual(3,f['outbound_matched_rows']);self.assertEqual(0,f['outbound_unknown_rows']);self.assertEqual(6,f['known_subset_value']);self.assertEqual(2,f['outbound_unknown_document_rows'])
    ...
    def test_sales_join_missing_or_duplicate_never_amplifies(self):
        ...
        f=self.value();self.assertIsNone(f['metric_value']);self.assertEqual(1,f['outbound_unknown_rows']);self.assertEqual(-3,f['known_subset_value'])
    ...
    def test_catalog_parameters_empty_scope_and_truncation(self):
        ...
        for kw in [{'movement_state':'New'},{'time_range':{'start':'2026-08-01','end':'2026-09-01'}},{'baseline_week':'2027-W01'},{'dimensions':['customer','unit']}]:self.assertEqual('failed',self.run_net(**kw)['status'])
