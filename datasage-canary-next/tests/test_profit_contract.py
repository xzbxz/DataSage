...
class ProfitContractTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for table in ('customer_profit_ads', 'delivery_bill_profit_ads'):
    ...
    def test_public_catalog_registers_two_ledgers_without_physical_fields(self):
        data=self.invoke('datasage_catalog',{'requests':[{'domain':'profit','view':'expert_index'}]})
        self.assertEqual('success',data['status'],data)
        self.assertEqual(35,len(data['results'][0]['metrics']),data)
