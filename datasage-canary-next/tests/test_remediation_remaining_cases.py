...
class RemainingCaseTests(unittest.TestCase):
    ...
    def test_B13_scorecard_availability_only(self):
        ...
        self.assertEqual('candidate_only',lens['status']);self.assertEqual({'department_month_gross_profit','customer_month_gross_profit','product_month_gross_profit','order_lifetime_gross_profit'},{x['metric']['code'] for x in lens['candidates']});self.assertFalse(self.sql_trace)
