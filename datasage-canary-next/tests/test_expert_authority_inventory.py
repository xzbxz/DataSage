...
class ExpertAuthorityInventoryTests(unittest.TestCase):
    ...
    def test_cross_domain_reference_states_its_compatibility_prerequisites(self) -> None:
        text = (
            SKILL_ROOT / "references/cross-domain-analysis.md"
        ).read_text(encoding="utf-8")
        lowered = text.lower()
        for prerequisite in ("period", "ledger", "grain", "unit", "currency", "department"):
            with self.subTest(prerequisite=prerequisite):
                self.assertIn(prerequisite, lowered)
        # It must keep refusing to become a planner or a fixed query sequence.
        self.assertIn("not a", lowered)
        for forbidden_role in ("planner", "sequence", "join layer"):
