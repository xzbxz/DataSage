...
class LegacyWorkflowTests(unittest.TestCase):
    ...
    def test_freeze_grain_threshold_repeat_and_current_policy_conflicts(self):
        ...
        reused=wf.freeze_plan([],plan['insert_rows'],'2026-W38','2026-W38')
        self.assertEqual('reuse_existing',reused['action'])
        repeated=wf.freeze_plan([source(5)],plan['insert_rows'],'2026-W38','2026-W38',refreeze=True)
        ...
        for rows in ([source(),source()],[source(source_unit=None)],[source(piece_num=None)],[]):
            self.assertEqual('blocked',wf.freeze_plan(rows,plan['insert_rows'],'2026-W38','2026-W38',refreeze=True)['status'])
