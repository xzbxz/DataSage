...
本文描述的实现已安装到本机 Profile，角色配置已按授权完成等价格式迁移。
本轮未发送消息，网关未重载；SQL SHA2 实体回查在真实数据库的扫描成本和延迟尚未测量。
...
文字和附件始终采用同一映射。业务角色由 Git 忽略的
`local/workflow-roles.json` 提供，旧角色参考只能显式导入；`target_map` 只描述最终
传输目标，不是角色规则来源，也不会触发旧路径回退。缺失角色或目标映射直接停止。
...
统一 `acceptance_delivery` 路径产生的每个新发送结果还带 `datasage-delivery-binding/v1`，绑定业务周期、代次、阶段、业务范围、
最终目标、正文/附件内容、组件 key 和 progress scope。只有 `verified_for_reuse` 才能作为
后续复用依据；缺少新 binding 的旧 `provider_accepted` 只属于
`historical_provider_accepted`，不能自动抑制发送。旧验收记录的 10/9 计数不能提升为新 binding 的通过。
