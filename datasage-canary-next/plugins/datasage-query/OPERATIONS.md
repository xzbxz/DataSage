...
| sales_prices | regions, limit; reference_source=legacy_database for legacy reminders; explicit profile_local only through the workflow entry | Current ready/promoted sales price identities; region source preferred over organization fallback. Latest-time ties stay ambiguous. Currency, source units, tax and validity changes are separate from price changes. Profile-local initialization remains a separate operator review; see WORKFLOW_IO.md. |
| purchase_prices | regions, limit; legacy_database stays read-only; explicit profile_local only through the purchase workflow entry | Product/color/supplier identities; included/excluded tax prices remain separate. Legacy organization scope retained. Missing source quote stays visible. Local reference uses the shared price engine; the user-supplied production robot binding remains disabled, with initialization and sending deferred. |
