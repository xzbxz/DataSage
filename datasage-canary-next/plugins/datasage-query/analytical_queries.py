...
def _frozen_pool_net_outbound_query(request, metric, datasets_contract, semantics, limit, *, observed_on=None, cohort=None):
    ...
    high_quantity_complete="outbound_unknown_rows=0 AND returns_unknown_rows=0 AND g.high_missing_price_rows=0 AND g.high_missing_quantity_rows=0 AND g.return_missing_quantity_rows=0 AND "+cohort_ok
    unit_high_quantity_complete="outbound_unknown_rows=0 AND returns_unknown_rows=0 AND u.high_missing_price_rows=0 AND u.high_missing_quantity_rows=0 AND u.return_missing_quantity_rows=0 AND "+cohort_ok
    ...
    total_fields+=f",CASE WHEN {high_quantity_complete} THEN g.high_known_gross_quantity-g.return_known_quantity ELSE NULL END AS high_net_quantity,g.high_known_gross_quantity-g.return_known_quantity AS high_known_net_quantity,CASE WHEN outbound_unknown_rows=0 AND g.high_missing_price_rows=0 AND g.high_missing_quantity_rows=0 AND {cohort_ok} THEN g.high_known_gross_quantity ELSE NULL END AS high_gross_quantity,CASE WHEN {unit_high_quantity_complete} THEN u.high_known_gross_quantity-u.return_known_quantity ELSE NULL END AS unit_high_net_quantity,u.high_known_gross_quantity-u.return_known_quantity AS unit_high_known_net_quantity"
