...
from . import idk_query, result_completeness

OperationError = idk_query.OperationError

FACT_FIELDS = idk_query.FACT_FIELDS

def build_idk_query(request,metric,datasets,semantics,limit,*,observed_on=None):
    return idk_query.build_idk_query(
        request,
        metric,
        datasets,
        semantics,
        ...
        table_quoting=_table,
    )

def policy():
    return idk_query.policy()

def _table(value):
    return idk_query._table(value)
