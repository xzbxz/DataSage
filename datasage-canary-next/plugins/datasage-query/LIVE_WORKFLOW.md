...
源为空、跨日、截断、超预算或源读取失败仍整批阻断。单键缺身份、歧义、币种/已知单位税口径变化、精度无法保存则单键隔离：保留旧参考并记录独立异常，其他健全键继续。按用户 2026-09-18 的明确选择，完整且身份可识别的观察中，离开监控池的旧键在必要回执完成后移出比较基线；重新进入时仅建立参考，不发送价格变化提醒。这不表示商品在业务上被删除。存在无法识别键的当前记录时，不能据此删除其他旧参考。已存在 origin 的空快照只在最近 committed 周期的 after_digest 证明它应为空时接受，不重新导入旧快照。
...
历史状态（2026-09-17）：当时用户已暂缓自动调度，原小时窗口仅留作历史提案，不视为当前授权或自动顺延。客户包全量本地检查及追加两包私信的历史记录位于运行 Home 的 `docs/manual-acceptance-checklist-20260917.md`；该材料未纳入源码审查白名单，需在获授权的原环境中查阅。本源码包不提供该历史材料，也不以其证明当前网关、客户包或投递状态。

本地全量客户包入口：`--mode live customer-audit` 读取当前周既有冻结并封存客户关系、独立对账与生成ZIP；`customer-repair` 仅修复封存的已知长字段渲染失败；`customer-coverage` 汇总当前周本地及发送覆盖，不重查来源。`customer-samples` 准备两个未抽验销售包；`customer-sample-send` 是真实外发入口，须先有对应内容和目标授权，已提交不重复发送，unknown保留待核。样本按ISO周隔离，不能把上周样本复用于下周。以上入口不会启用调度。

`--mode test manual-boundaries` 运行明确标识的合成周次/代次控制流检查，并核对保留的故障恢复证据；不伪造真实观察时间、不创建未来冻结、不发送消息。
